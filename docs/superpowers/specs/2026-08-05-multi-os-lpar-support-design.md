...
3. `linux` — `osType` is present and non-empty, and didn't match `ibmi` or `aix` above (covers `rhel`, `sles`, and any other distro codename PowerVS may report).
4. `other` — only when there is no `osType`, no `operatingSystem`, and no `softwareLicenses` signal at all (genuinely unclassifiable).
...
- `check_locally_VSI_exists` additionally reads `.os` for the matched system (`// "ibmi"` fallback per the backward-compatibility rule above) into a new global `vsi_os`.
