...
- `img_export()`: takes `IMGNAME BUCKET BUCKET_REGION CURRACCOUNT|OTHERACCOUNT [HMAC_JSON_FILE]`. Structure:
  ...
  3. Resolve COS credentials: `CURRACCOUNT` → existing globals `$accesskey`/`$secretkey`; `OTHERACCOUNT` → `load_hmac_keys`. **Invariant (user requirement, 2026-08-06): `$cos_accesskey`/`$cos_secretkey` are resolved exactly once, in this step, into local variables — the bucket pre-check in step 4 and the `ACTIONS` payload in step 5 both read these same two variables, never re-resolved or re-derived separately.** This guarantees the pre-validation HEAD request always authenticates with the identical HMAC pair that gets sent to PowerVS, so a pre-check "pass" can never mean something different from what PowerVS itself will see. (`img_import()` already has this property by construction — same pattern, made explicit here since it's easy to break by accident in a future edit, e.g. if someone re-reads `$hmac_file` a second time instead of reusing the resolved variables.)
  4. Pre-validate the **target bucket** (not a specific object — it doesn't exist yet) via an HTTP HEAD on the bucket root, stripping any `bucket/optional/folder` suffix from `BUCKET` first (`bucket_root="${export_bucket%%/*}"`) since S3 bucket-existence checks operate on the bucket itself. Same `--aws-sigv4`/bearer-auth branching as `img_import()`'s object-level HEAD check (using the step-3 `$cos_accesskey`/`$cos_secretkey`), same HTTP-status-to-message mapping (200/204 OK, 301/302/307/308 → likely wrong region, 403 → credentials, 404 → not found, else → generic). All failure branches here `abort ... 1` (see A.1).
