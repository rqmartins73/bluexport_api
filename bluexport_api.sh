...
cos_rest_arch() {
	...
	curl -sX POST "https://s3.$REGION.cloud-object-storage.appdomain.cloud/$bucket_name/$object_key?restore" -H "$header_auth" -H "$header_xml" --data-binary "$actions_xml" 2>>"$log_file" | tee -a "$log_file"
}
